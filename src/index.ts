import { render , html , ViewTemplate , ComponentContext , CSSProperties } from '@lithium-framework/core';
import { repeat , useState , style , asyncAppend , createRef , ref , until } from '@lithium-framework/core/dist/directives';
...
let blocks_style:CSSProperties = { padding : '10px' , border : '1px solid black' }
...
const RouterSample = html`<div style = ${style(blocks_style)}>
    <lithium-router :config = ${[
        { path : '/' , render : html`<h1>Hello</h1>` },
        { path : '/users/*' , render : html`<div>
            <h1>Users</h1>
            <lithium-router :config = ${[
                { path : '*/:userId' , render : html<{ userId:string }>`${(context) => {
